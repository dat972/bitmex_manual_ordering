...
class BitMEX(object):
    ...
    # added a redundant limit order function so code with def place_order wouldnt be broken
    @authentication_required
    def place_limit_order(self, quantity, price, side):
        """Place an order."""
        if float(price) < 0:
            raise Exception("Price must be positive.")
    ...
    @authentication_required
    def place_market_order(self, quantity, side):
        if float(quantity) % 0.5 != 0:
            print("Order Canceled Invalid Order Quantity. Lowest tick size is 0.5")
            return ("Invalid Order Quantity. Lowest tick size is 0.5")
